...
class DeepGlobeDataSet(data.Dataset):
    def __init__(self, root, list_path, module, crop_size=(320, 320), mean=(128, 128, 128), scale=False, mirror=False,
                 ignore_label=255):
        ...
        self.class_mappings = {'urban_land': 0, 'agriculture_land': 1, 'rangeland': 2, 'forest_land': 3, 'water': 4,
                               'barren_land': 5}
        print("1")

        for name in self.img_ids:
            # print(name)
            img_file = osp.join(self.root, "deepglobe/train/%s_sat.jpg" % name)

            if self.module == 's4gan':
                label_file = osp.join(self.root, "deepglobe/train/%s_mask.png" % name)
            else:
